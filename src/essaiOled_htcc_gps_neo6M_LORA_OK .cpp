...
*/
  ...
  char voltageDisplay[20]; // Tampon pour stocker la chaîne de caractères
  sprintf(voltageDisplay, "%d mV", batteryVoltage); // Formater la chaîne avec la valeur de batteryVoltage
  oledDisplay.drawString(64, 32 - 16 / 2, voltageDisplay); // Afficher la chaîne sur l'écran OLED
  ...
  if (gps.location.isValid()) {
    ...
    // Afficher les coordonnées GPS sur l'écran OLED
    if (gps.location.isValid()) {
      // Formater la latitude et la longitude dans deux chaînes distinctes
      char latitude[20];
      char longitude[20];
      sprintf(latitude, "lat: %d.%d", 
              (int)gps.location.lat(), 
              abs(fracPart(gps.location.lat(), 6)));
      sprintf(longitude, "lng: %d.%d", 
              (int)gps.location.lng(), 
              abs(fracPart(gps.location.lng(), 6)));
  
      // Afficher la latitude et la longitude sur deux lignes
      oledDisplay.setTextAlignment(TEXT_ALIGN_CENTER);
      oledDisplay.clear();
      oledDisplay.setFont(ArialMT_Plain_16);
      oledDisplay.drawString(64, 16, latitude);  // Afficher la latitude sur la première ligne
      oledDisplay.drawString(64, 32, longitude); // Afficher la longitude sur la deuxième ligne
      oledDisplay.display();
      delay(2000);
    } else {
      // Si les données GPS ne sont pas valides
      oledDisplay.setTextAlignment(TEXT_ALIGN_CENTER);
      oledDisplay.clear();
      oledDisplay.setFont(ArialMT_Plain_16);
